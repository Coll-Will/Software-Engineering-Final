<?php
  function randomManufacturer($db){
      ...
      if($query=$db->prepare("SELECT MID, street, city, state, zip, closest_WID
        FROM manufacturer WHERE MID = $MID")){
          $query->execute();
          $query->bind_result($mid, $street, $city, $state, $zip, $closest_WID);
          $query->fetch();
          $record = array($mid, $street, $city, $state, $zip, $closest_WID);
          $query->close();
      }else echo "Could not run query";
      return $record;
  }
