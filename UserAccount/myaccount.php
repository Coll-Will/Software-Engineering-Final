<?php
  ...
  if(session_status() === PHP_SESSION_ACTIVE){
      ...
      $orders = "";
      $query = "SELECT SID, cur_street, cur_city, cur_state, cur_zip, inTransit FROM shipments WHERE CID = $CID";
      if($result = mysqli_query($db, $query)){
          if(mysqli_num_rows($result) > 0){
            ...
            while($row = mysqli_fetch_array($result)){
                $orders = $orders . "<tr>";
                    $orders = $orders . "<td style='font-size:12; padding-top:8px'>" . $row['SID'] . "</td>";
                    if($row['inTransit'] === '1'){
                      $orders = $orders . "<td style='font-size:12; padding-left:16px; padding-top:8px'>" . $row['cur_street']
                      . ", " . $row['cur_city'] . ", " . $row['cur_state'] . " " . $row['cur_zip'] . "</td>";
                      $orders = $orders . "<td style='font-size:12; padding-left:16px; padding-top:8px'>" . "In-Transit". '</td>';
                    }
                    else {
                      $orders = $orders . "<td style='font-size:12; padding-left:16px; padding-top:8px'>" . "N/A" . "</td>";
                      $orders = $orders .  "<td style='font-size:12; padding-left:16px; padding-top:8px'>" . "Processing". '</td>';
                    }
                $orders = $orders . "</tr>";
            }
            $orders = $orders . "</table>";
     ...
     <body>
          <div class="container">
                   <div style="display: flex; padding: 5%;">
                        <div class="row" style="flex: 1; margin-right: 5%; padding: 5%; border: 2px solid black; align-items: center; justify-content: center">
                        ...
                        <div style="flex: 1; margin-right: 5%; padding: 5%; border: 2px solid black; text-align: center;">
